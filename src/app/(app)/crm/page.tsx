...
export default async function CrmPage({
  ...
  const missingData = leads.filter(
    (l) => l.outcome === "open" && (!l.next_action || !l.next_action_date || !l.owner_id),
  ).length;
  ...
  return (
    <>
      <PageHeader
        title="CRM"
        description="Pipeline comercial de NetFlow. Ninguna oportunidad abierta puede quedar sin responsable, estado y proxima accion."
      >
      ...
      {missingData > 0 && (
        <p className="mb-4 rounded-lg border border-warn-soft bg-warn-soft px-3 py-2 text-sm text-warn">
          {missingData} oportunidad(es) abiertas sin proxima accion definida. Son las primeras que hay que ordenar.
        </p>
      )}
